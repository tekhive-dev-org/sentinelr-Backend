...
exports.generatePairingCode = catchAsync(async (req, res) => {
  ...
  try{
      ...
      const expiresAt = new Date(new Date().setFullYear(new Date().getFullYear() + 2))
...
exports.pairDevice = catchAsync(async (req, res) => {
  ...
  try{
    ...
    await pairingCode.update({ status: 'Paired', deviceId: device.id, userId: device.userId, usedAt: new Date() }, { transaction })
    await transaction.commit()

    const deviceToken = jwt.sign({ deviceId: device.id, deviceName: device.deviceName }, process.env.DEVICE_SECRET, { expiresIn: '365d' })

    res.status(201).json({ success: true, message: 'Device paired successfully', deviceId: device.id, deviceUserId: device.userId, deviceToken })
  }
  catch(error){
